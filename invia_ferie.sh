...
if [ "$FORZA" != "true" ]; then
  # Il job può essere fatto partire da GitHub con qualche minuto di anticipo
  # apposta (vedi cron nel workflow). Una volta che il job è già acceso,
  # possiamo aspettare noi stessi, con un timer preciso al secondo, fino
  # all'istante esatto delle 22:00:00 — molto più affidabile che sperare
  # che il cron scatti puntuale (GitHub stessa avverte che gli scheduled
  # workflow possono ritardare, specialmente "in cima all'ora").
  EPOCH_TARGET=$(TZ='Europe/Rome' date -d "${DATA_ROMA} ${ORA_TARGET}:${MINUTO_TARGET}:00" +%s)
  EPOCH_ADESSO=$(date +%s)

  if [ "$EPOCH_ADESSO" -lt "$EPOCH_TARGET" ]; then
    ATTESA=$(( EPOCH_TARGET - EPOCH_ADESSO ))
    echo "Sono le $(TZ='Europe/Rome' date +%H:%M:%S) ora italiana. Aspetto ${ATTESA} secondi, fino esattamente alle ${ORA_TARGET}:${MINUTO_TARGET}:00..."
    sleep "$ATTESA"
    echo "Sono le $(TZ='Europe/Rome' date +%H:%M:%S) ora italiana. Invio adesso."
  else
    RITARDO=$(( EPOCH_ADESSO - EPOCH_TARGET ))
    echo "::warning::Il job è partito con ${RITARDO} secondi di ritardo rispetto alle ${ORA_TARGET}:${MINUTO_TARGET}:00 (probabile coda di GitHub Actions). Invio subito, il prima possibile."
  fi
else
  echo "FORZA attivo: salto l'attesa e invio subito."
fi

DATA_FERIE=$(TZ='Europe/Rome' date -d "${DATA_ROMA} +30 days" +%F)

# Conversione in due passaggi: prima l'epoch (assoluto, nessuna ambiguità di fuso),
# poi l'epoch in UTC. Fare "-u" insieme a "-d" con un orario locale in un solo
# comando NON converte il fuso: stampa l'orario as-is spacciandolo per UTC.
EPOCH_FERIE=$(TZ='Europe/Rome' date -d "${DATA_FERIE} 00:00:00" +%s)
DATA_ISO=$(date -u -d "@${EPOCH_FERIE}" +"%Y-%m-%dT%H:%M:%S.000Z")
